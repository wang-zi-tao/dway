...
pub fn spawn(trigger: Trigger<OnAdd, DWayServer>, query: Query<&DWayServer>) {
    if let Ok(compositor) = query.get(trigger.entity()) {
        for _i in 0..1 {
...
pub fn spawn_x11(trigger: Trigger<DWayXWaylandReady>, query: Query<&DWayServer>) {
    if let Ok(_compositor) = query.get(trigger.event().dway_entity) {
