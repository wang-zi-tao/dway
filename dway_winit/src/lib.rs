...
impl Plugin for WinitPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_non_send_resource::<WinitWindows>()
            ...
            .init_non_send_resource::<UpdateRequestEvents>()
            .set_runner(winit_runner)
            .add_schedule(FrameConditionSchedule, Schedule::new())
...
pub struct UpdateRequestEvents {
    pub receiver: Receiver<UpdateRequest>,
    pub sender: Sender<UpdateRequest>,
}

impl Default for UpdateRequestEvents {
    fn default() -> Self {
    ...
    let event_handler = move |event: Event<()>,
        ...
        match event {
            ...
            event::Event::MainEventsCleared => {
                app.world.run_schedule(FrameConditionSchedule);
                ...
                let has_update_request = if let Some(update_request_events) =
                    app.world.get_non_send_resource::<UpdateRequestEvents>()
                {
                    let event_count = update_request_events.receiver.try_iter().count();
                    event_count > 0
                } else {
                    false
                };
