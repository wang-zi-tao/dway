...
fn vertex(@builtin(vertex_index) vertex_index: u32) -> VertexOutput {
    ...
    let f = 1.0 + settings.layer;

    let p0 = uv + vec2(1.0,1.0) * settings.radius * f / position.size;
    let p1 = uv + vec2(1.0,-1.0) * settings.radius * f / position.size;
    let p2 = uv + vec2(-1.0,1.0) * settings.radius * f / position.size;
    let p3 = uv + vec2(-1.0,-1.0) * settings.radius * f / position.size;
